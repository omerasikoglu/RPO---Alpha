...
public class PlayerController : MonoBehaviour, ICollider, IShopCustomer
{
    ...
    {
        xInput = Input.GetAxisRaw("Horizontal");    //yürüme

        if (Input.GetKeyDown(KeyCode.X) && canDash && CanUseDash()) //dash atma
        {
        ...
        if (isGrounded)
        {
            ...
            if (Mathf.Abs(rigidbody2d.velocity.x) >= .01f) //yürüyor
            {
                isWalking = true;
            }

            else if (xInput == 0) //duruyor
            {
                isWalking = false;
            }
        ...
        else if (!isGrounded)   //yere değmiyorsa
        {
            ...
            if (onRightWall)
            {
                ...
                if (xInput == 1 && rigidbody2d.velocity.y < -2f && !isClimbingLedge) //duvarda sağa tıklıyorsa
                {
                    wallSliding = true;
                }
                else
    ...
    private void ApplyMovement()
    {
        if (canMove)
        {
            if (isGrounded && !isKnockbacking) //yürüme hızı
            {
                rigidbody2d.velocity = new Vector2(movementSpeed * xInput, rigidbody2d.velocity.y);
            }
            else if (!isGrounded && !wallSliding && !isKnockbacking) //havada yürüme hızı
            {
